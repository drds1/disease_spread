...
def run_govukmodel(save_output=True,
    ...
    if save_output:
        ...
        if os.path.exists(dirname) is False:
            os.makedirs(dirname)
        pdf = matplotlib.backends.backend_pdf.PdfPages(os.path.join(dirname,"rmodel_outputs.pdf"))
        pdf.savefig(fig_plot)
        pdf.savefig(fig_covariance)
        pdf.close()
        f = open(os.path.join(dirname,"model.pkl"), "wb")
...
def perform_1it():
    ...
    x.fit()
    x.get_output_parms()
    fig_plot = x.plot_model(file=None,#'rvalue_forecast.pdf',
                            return_figure=True,
                            reference_date=pd.Timestamp(2020, 9, 1))
    fig_covariance = x.plot_covariance(file=None,#'covariance_plot.pdf',
                                       return_figure=True)
